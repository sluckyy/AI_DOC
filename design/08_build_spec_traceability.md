...
| N-1 barge-in, N-3 endpointing, N-9 per-utterance confidence | Met | Azure recognition in the browser (D-61): segmentation silence from `silence_end_of_turn_ms` and `silence_end_of_turn_ms_by_register`; confidence and n-best stored per turn (`Turn.asr`), summarised in the handover metadata and narrative, low-confidence turns listed against `asr_low_confidence_threshold` |
| N-8 per-cohort ASR accuracy | Partial | Confidence is recorded per utterance and per language; cohort reporting needs the study's cohort labels and a reference transcript |
